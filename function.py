...
def CheckGetBlind(url, blindlist, html):
    ...
    if (blindtrue == html) and (blindtrue != blindfalse):
        bar.printabove(
            colored("[GET] ", "yellow", attrs=["bold"]) + colored(url, "yellow") + colored(blindlist[1], "red",
                                                                                           attrs=["bold"]))
...
def CheckGetVuln(url, vuln, html):
    ...
    for ivalidproof in validproof:
        if (ivalidproof in page) or (currenttested == "timebase" and (endtime - starttime) >= (reponsetime + 2)):
            bar.printabove(colored("[GET] ", "green", attrs=["bold"]) + colored(url, "green") + colored(vuln, "red",
                                                                                                        attrs=["bold"]))
            return payloadedurl
    return False
...
def CheckPostBlind(url, blindlist, fields, html):
    ...
    if (blindtrue == html) and (blindtrue != blindfalse):
        ...
        printpayload = printpayload + colored("}", "yellow")
        bar.printabove(
            colored("[POST] ", "yellow", attrs=["bold"]) + colored(url, "yellow") + colored(" [PARAM] ", "yellow",
                                                                                            attrs=[
                                                                                                "bold"]) + printpayload)
...
def CheckPostVuln(url, vuln, fields, html):
    ...
    for ivalidproof in validproof:
        if (ivalidproof in page) or (currenttested == "timebase" and (endtime - starttime) >= (reponsetime + 2)):
            ...
            printpayload = printpayload + colored("}", "green")
            bar.printabove(
                colored("[POST] ", "green", attrs=["bold"]) + colored(url, "green") + colored(" [PARAM] ", "green",
                                                                                              attrs=[
                                                                                                  "bold"]) + printpayload)
